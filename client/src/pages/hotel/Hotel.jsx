...
import ReserveUser from "../../components/reserve/ReserveUser";
...
const Hotel = () => {
  const location = useLocation(); // Object Path
  const hotel_id = location.pathname.split("/")[2];

  const [slideNumber, setSlideNumber] = useState(0);
  const [open, setOpen] = useState(false);
  const [openModal, setOpenModal] = useState(false);
  const { data, loading } = useFetch(`/hotels/find/${hotel_id}`);
  ...
  function day_Difference(date_1, date_2) {
    const time_diff = Math.abs(date_2.getTime() - date_1.getTime());
    const diff_days = Math.ceil(time_diff / MILLISECONDS_PER_DAY);
    return diff_days;
  }
  const num_days = day_Difference(dates[0].endDate, dates[0].startDate);
  const handleOpen = (i) => {
    setSlideNumber(i);
    setOpen(true);
  };
  ...
  return (
    <div>
          ...
          <div className="hotelWrapper">
            ...
            <div className="hotelDetails">
              ...
              <div className="hotelDetailsPrice">
                <h1>Perfect for a {num_days}-night stay!</h1>
                <span>
                  Located in the real heart of Krakow, this property has an
                  excellent location score of 9.8!
                </span>
                <h2>
                  <b>${num_days * data.cheapestPrice * options.room}</b> (
                  {num_days} nights)
                </h2>
      ...
      {openModal && <ReserveUser setOpen={setOpenModal} hotelId={hotel_id} />}
    </div>
  );
};
